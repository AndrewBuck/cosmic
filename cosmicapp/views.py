...
def processes(request, process=None):
    ...
    validPages = ['astrometrynet', 'generatethumbnails', 'imagestats', 'parseheaders', 'sextractor', 'image2xy',
        'daofind', 'starfind', 'starmatch', 'flagsources', 'imagecombine', 'calculateusercosttotals']
...
def imageProperties(request, id):
    ...
    properties = ImageProperty.objects.filter(image_id=image.pk).order_by('key', 'header__index')
...
def combineImageIds(request):
    ...
    lightCombineMethod = request.POST.get('lightCombineMethod', "")
    ...
    totalSize = 0
    filteredIdList = ['combineType=str:' + combineType, 'lightCombineMethod=str:' + lightCombineMethod]
